DEFAULT_OUT_PATH = 'domains/generated/problems/'
MIDDLE = '''
	)
(:init
'''
...
def connect_tiles(tiles, direction, reverse_direction):
    ...
    for current_tile, next_tile in zip(tiles, tiles[1:]):
        predicates.append(
            '({} {} {})'.format(direction, current_tile, next_tile))
        predicates.append('({0} {2} {1})'.format(
            reverse_direction, current_tile, next_tile))
    return predicates


def generate_corridor(length, path=None):
    if path is None:
        path = '{}/corridor_{}.pddl'.format(DEFAULT_OUT_PATH, length)
...
def generate_T(corridor_length, goal_length, deadend_length, path=None):
    '''
    ...
    '''
    if path is None:
        path = '{}/t_{}_{}_{}.pddl'.format(
            DEFAULT_OUT_PATH, corridor_length, goal_length, deadend_length)
